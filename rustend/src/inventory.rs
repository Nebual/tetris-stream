...
#[get("/")]
pub fn index(conn: DbConn) -> QueryResult<Json<Vec<Inventory>>> {
...
#[get("/?<params..>")]
pub fn search(conn: DbConn, params: Form<SearchParams>) -> QueryResult<Json<Vec<Inventory>>> {
...
#[get("/<inv_id>")]
pub fn get(conn: DbConn, inv_id: i32) -> Option<JsonValue>    {
    assert!(inv_id > 0);
...
#[post("/", data = "<new>")]
pub fn create(conn: DbConn, new: Json<NewInventory>) -> QueryResult<Json<Inventory>> {
...
pub fn update(conn: DbConn, inv_id: i32, new: Json<NewInventory>) -> QueryResult<Json<Inventory>> {
    assert!(inv_id > 0);
...
#[delete("/<inv_id>")]
pub fn delete(conn: DbConn, inv_id: i32) -> Result<Status, diesel::result::Error> {
    assert!(inv_id > 0);
    diesel::delete(inventory::table.find(inv_id)).execute(&*conn)?;
    Ok(Status::NoContent)
}
...
pub fn change_game(
    conn: DbConn,
    ws_connections: State<WSSocketDataMutexVec>,
    inv_id: i32,
    new: Json<InventoryGameChange>,
) -> QueryResult<Json<Inventory>> {
    assert!(inv_id > 0);
...
#[get("/<inv_id>/items")]
pub fn get_items(conn: DbConn, inv_id: i32) -> Option<Json<Vec<InventoryItem>>> {
...
pub fn move_item(
    conn: DbConn,
    dest_inv_id: i32,
    req: Json<MoveItemRequest>,
    ws_connections: State<WSSocketDataMutexVec>,
) -> Result<Status, status::NotFound<String>> {
...
fn broadcast_inventory_changes(
    ...
    let source_inv_items = //c::defer(||
                                            InventoryItem::get_in_inventory(&conn, source_inv_id).unwrap_or(Vec::new());
    let dest_inv_items = //Thunk::defer(||
        InventoryItem::get_in_inventory(&conn, dest_inv_id).unwrap_or(Vec::new());
    if let Ok(ws_connections) = ws_connections.lock() {
